...
int main(int argc, char *argv[]) {
    ...
    INFO_PRINTF("vision app classification start\n");
    ...
    while (!s_is_evp_exit) {
        ...
        ret = PPL_ClassificationAnalyze((float *)raw_data.address, raw_data.size/4, &p_out_buf, &out_size, s_cls_param);

        if ((EPPL_RESULT_CODE)ret != E_PPL_OK) {
            ERR_PRINTF("PPL_ClassificationAnalyze : ret=%d", ret);
        }
        else {
...
static void ConfigurationCallback(const char *topic, const void *config, size_t configlen, void *userData) {
    ...
    if (str_ret == 0) {
        ...
        INFO_PRINTF("dnn_output_Classes: %d  max_predictions: %d", \
            s_cls_param.dnnOutputClasses, s_cls_param.maxPredictions);
        return;
    }
    ...
    EPPL_RESULT_CODE ret = json_parse(root_value, &s_cls_param);
    if (ret != E_PPL_OK) {

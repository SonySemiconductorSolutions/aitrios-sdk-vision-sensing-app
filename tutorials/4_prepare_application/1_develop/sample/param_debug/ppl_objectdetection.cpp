...
static EPPL_STATUS s_ppl_status = PPL_INIT;
...
static char *sp_param;
static int s_err_buf = 0;
static bool s_is_param_parsed = false;
...
static EPPL_RESULT_CODE PPL_ParseParam(void);
...
EPPL_RESULT_CODE PPL_Initialize(uint32_t network_id, const char *p_param) {
    ...
    if (s_ppl_status != PPL_INIT) {
        PPL_ERR_PRINTF("PPL:Invalid state : current state=0x%02X", s_ppl_status);
        return E_PPL_OK;
    }
    int param_len = strlen(p_param);
    if (0 < param_len) {
        sp_param = (char *)malloc(param_len);
        if(sp_param != NULL) {
            memcpy(sp_param, p_param, param_len);
        }
    }
    else {
        char default_param[256];
        snprintf(default_param, 256, "{\"header\" :{\"id\" : \"%s\",\"version\" : \"%s\"},\"dnn_output_detections\" : %d,\"max_detections\" : %d,\"threshold\" : %f,\"input_width\" : %d,\"input_height\" : %d}", PPL_ID, PPL_VERSION, PPL_DNN_OUTPUT_DETECTIONS, PPL_DNN_OUTPUT_DETECTIONS, PPL_DEFAULT_THRESHOLD, PPL_SSD_INPUT_TENSOR_WIDTH, PPL_SSD_INPUT_TENSOR_HEIGHT);
        sp_param = (char *)malloc(strlen(&default_param[0]));
        if(sp_param != NULL) {
            memcpy(sp_param, default_param, strlen(&default_param[0]));
        }
    }
    s_ppl_status = PPL_IDLE;
    PPL_DBG_PRINTF("PPL state: PPL_IDLE");
    return E_PPL_OK;
}
...
EPPL_RESULT_CODE PPL_Analyze(float *p_data, uint32_t in_size, void **pp_out_buf, uint32_t *p_out_size, bool *p_upload_flag) {
    ...
    s_ppl_status = PPL_DONE;
    PPL_DBG_PRINTF("PPL state: PPL_DONE");
    return ret;
}
...
EPPL_RESULT_CODE PPL_Finalize() {
    ...
    s_is_param_parsed = false;
    s_ppl_status = PPL_INIT;
    PPL_DBG_PRINTF("PPL state: PPL_INIT");
    return E_PPL_OK;
}
...
static EPPL_RESULT_CODE PPL_ParseParam(void) {
    ...
    if (strcmp(sp_param, "") == 0) {
        PPL_INFO_PRINTF("PPL_ParseParam: PPL parameter is empty. Using default values:");
        ...
        root_value = json_parse_string(sp_param);
        if(json_value_get_type(root_value) != JSONObject) {
            json_value_free(root_value);
            PPL_ERR_PRINTF("PPL_ParseParam: param is not Json object");
            return E_PPL_INVALID_PARAM;
        }

        ret = json_parse(root_value);
        if(ret != E_PPL_OK){
            PPL_ERR_PRINTF("PPL_ParseParam: Get json_parse Fail Err[%d]", ret);
            json_value_free(root_value);
            return ret;
        }
        json_value_free(root_value);
